...
.ra_write_rd_exports <- function(base_path, exports, code) {
  ...
  argnames <- character()
  for (nm in exports) {
    pat <- paste0("^", nm, " <- function\\((.*)\\) \\{")
    line <- grep(pat, code, value = TRUE)
    args <- if (length(line) > 0L) sub(pat, "\\1", line[[1]]) else "value"
    usages <- c(usages, paste0(nm, "(", args, ")"))
    if (nzchar(args)) {
      parts <- trimws(sub("=.*$", "", trimws(strsplit(args, ",")[[1]])))
  ...
  if (length(argnames) > 0L) {
    lines <- c(
      lines,
      "\\arguments{",
      sprintf("\\item{%s}{Argument(s) / value validated by the helper(s).}", paste(argnames, collapse = ", ")),
      "}"
    )
  }
  lines <- c(
    lines,
    "\\value{",
    "\\code{assert_args_*} return \\code{invisible(NULL)}; \\code{assert_return_*} and",
    "\\code{assert_type_*} return the validated value invisibly. Called for the side",
    "effect of signalling an error on a contract violation.",
    "}",
    "\\description{",
