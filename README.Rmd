...
- **`@yesassert`** — on a block defining one or more `@type`, emit a standalone `assert_type_<Name>(value)` for each, even if nothing references the type. It is the mirror of `@noassert` (*don't* generate a check here / *do* generate one here).
- **`@exportassert`** — export the asserts generated **from that block** so other packages can call them. Orthogonal to `@yesassert`, and it works on a function or R6 method block too (exporting its `assert_args_*` / `assert_return_*`). Distinct from roxygen2's `@export`, which exports the documented object, not its assert helpers.
...
`@exportassert` appends a managed `export(...)` block to the package `NAMESPACE` **and** writes a `\keyword{internal}` Rd documenting the exported helpers (R requires exported objects to be documented, so this keeps `R CMD check` clean). Both are rewritten deterministically on each `document()`.
