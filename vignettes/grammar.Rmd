...
6. **Values are copied verbatim; the generator never coerces.** Every value the
   ...
   and taken on trust for opaque expressions (S2). **The one exception** is a
   `class<pkg::Name>` reference: its `pkg::` qualifier is documentary and is
   stripped to the final segment for the generated `assert_class()` (R's class
   system is flat), so it is the sole place roxyassert rewrites your input rather
   than emitting it verbatim (§6, reference types).
...
S7), matches subclasses (`class<AbstractClock>` accepts a `RealClock`), and
accepts a documentary `pkg::` qualifier (`class<lubridate::Duration>`, dropped
from the generated check). A composite is refined by nested bullets (a named
...
reference      ::= "function" | "class" "<" qualified_ident ">"
...
qualified_ident ::= ident ( "::" ident )*
                   (* a class name, optionally package-qualified; the `pkg::`
                      qualifier is documentary and is dropped from the generated
                      assert_class() check, which uses the final segment *)
...
- **`>`** closes the innermost open generic (depth-aware); `vector<...>` and
  `class<...>` each close one level. `class` must be followed by `<Name>` (a
  `qualified_ident`); a bare `class` is a parse error.
...
- **Everything after `)` is free-text description.** roxyassert parses only the
  ...
  Names contain no spaces. Child `bullets` begin only on a *subsequent* line at
  strictly greater indentation whose first non-space characters are `- `; a `- `
...
| `class<Class>` | reference | an object whose class is `Class` — any object system (S3/S4/RC/R6/S7), subclasses match; `class<pkg::Class>` allowed (length 1) |
...
(class<lubridate::Duration>)         # qualifier is documentary; checks "Duration"
...
- [ ] `function`/`class<Class>` appear only as bare reference types; `class` always names a class (any object system; `class<pkg::Name>` accepted, qualifier dropped from the check).
